...
| 항목 | 뜻 |
| --- | --- |
| `branch_no` | 지점 번호. 구갈점은 `3` |
| `branch_name` | 알림에 표시할 지점 이름 |
| `target_product_name` | 감시할 장난감 이름 |
| `target_product_no` | 감시할 대표 상품번호. 비워두면 검색으로 찾습니다 |
| `search_keyword` | 상품번호를 비웠을 때 검색에 쓸 단어 |
| `check_interval_seconds` | 한 실행 안에서 몇 초 간격으로 다시 확인할지 |
| `checks_per_run` | 한 번 실행될 때 몇 번 확인할지 |
| `notify_when_available` | 가능한 장난감이 있을 때 알림 보낼지 |
| `notify_when_no_available` | 가능한 장난감이 없어도 확인 완료 알림 보낼지 |
| `include_status_list` | 알림에 전체 바코드별 상태를 같이 넣을지 |
...
전체 대여상태:
- 3301551: 대여중 / 반납예정일 2026-06-12
- 3301550: 대여중 / 반납예정일 2026-06-13
- 3301549: 대여가능

링크: https://www.yicare.or.kr/main/main.php?categoryid=23&menuid=03&groupid=02
...
```powershell
cd "C:\Users\김경호\Documents\장난감도서관 확인"
python -m pip install -r requirements.txt
python check_yicare_toy.py
```

반복 실행까지 테스트하려면:

```powershell
python check_yicare_toy.py --loop
```
