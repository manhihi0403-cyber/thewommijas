...
더우미제 돌잔치 예약 페이지에서 `2026년 8월 1일 party` 예약 상태를 확인하고 텔레그램 알림을 보내는 봇입니다.
...
- `마감`이어도 확인 완료 알림을 보냅니다.
- 자리가 나면 예약 가능 알림을 보냅니다.
...
현재 상태가 `마감`이면 이런 식으로 로그가 나오고 텔레그램 확인 완료 알림도 보냅니다.
...
이 봇은 실행될 때마다 텔레그램을 보냅니다.

현재 상태가 `마감`이면 아래처럼 확인 완료 메시지가 옵니다.
...
실제 사이트 상태가 `마감`이어도 확인 완료 메시지는 옵니다.
...
1. 봇에게 먼저 아무 메시지나 보냈는지 확인합니다.
2. `TELEGRAM_CHAT_ID`가 정확한지 확인합니다.
3. GitHub Actions 로그에 `텔레그램 알림을 발송했습니다.`가 찍혔는지 확인합니다.
